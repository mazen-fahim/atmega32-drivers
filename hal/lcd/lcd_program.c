...
void LCD_VoidPrintCharXY(u8 Copy_U8Char, u8 Copy_U8X, u8 Copy_U8Y)
{
    if (Copy_U8Y == 0)
    {
        LCD_VoidSendCommand(0b10000000 + 0x40 + Copy_U8X); // 0x40 to start in the second line y == 0
        LCD_VoidSendData(Copy_U8Char);
    }
    else if (Copy_U8Y == 1)
    {
        LCD_VoidSendCommand(0b10000000 + Copy_U8X);
        LCD_VoidSendData(Copy_U8Char);
    }
}
...
void LCD_VoidPrintStringXY(u8 *Copy_U8String, u8 Copy_U8X, u8 Copy_U8Y)
{
    ...
    {
        LCD_VoidSendCommand(0b10000000 + 0x40 + Copy_U8X); // 0x40 to start in the second line y == 0
        LCD_VoidPrintString(Copy_U8String);
    }
    else if (Copy_U8Y == 1)
    {
        LCD_VoidSendCommand(0b10000000 + Copy_U8X);
...
void LCD_VoidSendNumber(u32 Copy_U32Number)
{
    ...
    else
    {
        ...
        while (LOC_U32ReversedNumber > 1)
        {
            ...
            LCD_VoidPrintChar(LOC_U32Number + '0');
...
u8 LCD_U8LoadCustm8ByteChar(u8 *symbol)
{
    ...
    u8 LOC_U8AdressCounterTemp = LCD_U8ReadAdressCounter();
    /* set the AC to point to the appropriate address in CG RAM*/
    LCD_VoidSendCommand(0b01000000 + 8 * CGRamCharCounter);
