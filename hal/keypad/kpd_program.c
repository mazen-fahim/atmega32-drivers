...
u8 KPD_U8GetPressedKey(void) {
  ...
  /* this differs for every application so we have to provide a way for the user
   * to configure its values*/
  static u8 LOC_U8Arr[4][4] = KPD_ARR_VAL;

  static u8 LOC_U8KPDColumnArr[COLUMN_NUM] = {KPD_COLUMN0_PIN, KPD_COLUMN1_PIN,
                                              KPD_COLUMN2_PIN, KPD_COLUMN3_PIN};
  static u8 LOC_U8KPDRowArr[ROW_NUM] = {KPD_ROW0_PIN, KPD_ROW1_PIN,
                                        KPD_ROW2_PIN, KPD_ROW3_PIN};

  for (LOC_U8ColumnIndex = 0; LOC_U8ColumnIndex < COLUMN_NUM;
       LOC_U8ColumnIndex++) {
    ...
    for (LOC_U8RowIndex = 0; LOC_U8RowIndex < ROW_NUM; LOC_U8RowIndex++) {
      ...
      LOC_U8PinState =
          MDIO_U8GetPinValue(KPD_PORT, LOC_U8KPDRowArr[LOC_U8RowIndex]);
